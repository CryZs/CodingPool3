<?php
    ...
    switch($case){
        ...
        case 'most_popular':
            ...
            $stmt = $pdo->prepare("SELECT AVG(comment.rate) AS moyenne, post.id, post.title, LEFT(post.body,150) AS extrait, user.name AS author FROM `comment` LEFT JOIN post ON post.id = comment.post_id LEFT JOIN user ON post.author_id = user.id GROUP BY post.title ORDER BY moyenne DESC LIMIT 5 OFFSET $offset");
            $stmt->execute();
            $posts = $stmt->fetchAll();
            $posts = json_encode($posts);
            header('Content-Type: application/json');
